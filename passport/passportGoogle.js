...
module.exports = function(passport) {
    ...
    passport.use(
      new GoogleStrategy({
        ...
        const existingUsersQuery = `SELECT * FROM ${process.env.DB_NAME}.users WHERE id = ?`;
        ...
        try{
            ...
            if(existingUsers.length > 0){
                ...
                try{
                    const addUserQuery = `INSERT INTO ${process.env.DB_NAME}.users (id, username, email, isGoogle, first_name, last_name, profile_img) VALUES (?,?,?,?,?,?,?)`;
                    await queryPromise(addUserQuery, [user.id, user.username, user.email, 1, user.first_name, user.last_name, user.profile_img]);
                    await queryPromise(`INSERT INTO ${process.env.DB_NAME}.custom_biomarkers (user_id, custom_biomarker_list) VALUES (?,?)`, [user.id, JSON.stringify([])]);
