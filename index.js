...
async function configureAndStartServer() {
  try {
    ...
    console.log("Secret Store Error...");
    throw err;
  }
  ...
  app.use(cookieParser(SecretStore.GetSecret("SECRET_COOKIE_SESSION_KEY")));

  app.use(
    expressSession({
      secret: SecretStore.GetSecret("SECRET_COOKIE_SESSION_KEY"),
      resave: false,
      ...
      cookie: {
        ...
        domain: isProduction() ? ".bloodsense.online" : "localhost",
        maxAge: parseInt(SecretStore.GetSecret("MAX_COOKIE_AGE_MILLIS")),
      },
  ...
  app.use(
    cors({
      origin: SecretStore.GetSecret("BASE_CLIENT_URL"),
      methods: ["GET", "POST"],
      allowedHeaders: [
        "Content-Type",
        "Authorization",
  ...
  app.get("/", (req, res) => {
    db.query("SELECT NOW()", (err, result) => {
      ...
      res.send(
        `Whats Up? DB seems to be working fine  ${SecretStore.GetSecret(
          "TESTVAR"
        )}`
      );
    });
